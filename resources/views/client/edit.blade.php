...
@section('content')
    ...
    <form method="POST" action="{{ route('update', ['id' => $blog->id]) }}">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="content" name="content" rows="5" placeholder="กรอกเนื้อหาบทความ">{{ old('content', $blog->content) }}</textarea>
            </textarea>
        </div>
        @error('content')
            <div class="alert alert-danger mt-3">{{ $message }}</div>
        ...
        <div class="form-group">
            ...
            <select class="form-select" id="status" name="status">
                <option value="draft" {{ old('status', $blog->status) == 'draft' ? 'selected' : '' }}>ฉบับร่าง</option>
                <option value="published" {{ old('status', $blog->status) == 'published' ? 'selected' : '' }}>เผยแพร่
                </option>
            </select>
        </div>

        <button type="submit" class="btn btn-primary my-3">บันทึกการแก้ไข</button>
